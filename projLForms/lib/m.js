if (typeof pas2js === "undefined") {
    pas2js = {};
}

pas2js.dialogs = (function (params) {
...
app.modal = function (params) {
    ...
    */
    ...
    var modal = _(app._modalTemlateTempDiv).children();
	console.log(_(app._modalTemlateTempDiv));
    ...
    return app.modal({
        ...
        buttons: [
            {text: app.params.modalButtonCancel, onClick: function(modal){
                if (callbackCancel) callbackCancel(_(modal).find('.modal-prompt-input').val());
            }},
            {text: app.params.modalButtonOk, bold:true, onClick: function(modal){
                if (callbackOk) callbackOk(_(modal).find('.modal-prompt-input').val());
            }}
        ]
    });
};
...
app.prompt = function (text, title, callbackOk, callbackCancel) {
    ...
    return app.modal({
        ...
        onClick: function (modal, index) {
            if (index === 0 && callbackCancel) callbackCancel(_(modal).find('.modal-text-input').val());
            if (index === 1 && callbackOk) callbackOk(_(modal).find('.modal-text-input').val());
        }
    });
};
*/
        ...
        buttons: [
            {
				...
				onClick: function(modal){
                  if (callbackCancel) callbackCancel(_(modal).find('.modal-text-input[name="modal-username"]').val(), _(modal).find('.modal-text-input[name="modal-password"]').val());
				}
            },
            {
                text: app.params.modalButtonOk,
                bold: true, 
				onClick: function(modal){
                  if (callbackOk) callbackOk(_(modal).find('.modal-text-input[name="modal-username"]').val(), _(modal).find('.modal-text-input[name="modal-password"]').val());
				}
            }
        ]/*,
        onClick: function (modal, index) {
            var username = _(modal).find('.modal-text-input[name="modal-username"]').val();
            var password = _(modal).find('.modal-text-input[name="modal-password"]').val();
        ...
        buttons: [
            {
                text: app.params.modalButtonCancel,
				onClick: function(modal){
                  if (callbackCancel) callbackCancel(_(modal).find('.modal-text-input[name="modal-password"]').val());
				}
            },
            {
                text: app.params.modalButtonOk,
                bold: true,
				onClick: function(modal){
                  if (callbackOk) callbackOk(_(modal).find('.modal-text-input[name="modal-password"]').val());
				}				
            }
        ]/*,
        onClick: function (modal, index) {
...
app.showIndicator = function () {
  _('body').append('<div class="preloader-indicator-overlay"></div><div class="preloader-indicator-modal"><span class="preloader preloader-white"></span></div>');
  //if (_('.preloader-indicator-overlay').length > 0) return;
...
app.hideIndicator = function () {
  _('.preloader-indicator-overlay, .preloader-indicator-modal').remove();
};
...
app.actions = function(params) {
    ...
    */
    ...
    groups.each(function (index, el) {
        ...
        _(el).find('.actions-modal-button').each(function (index, el) {
...
app.openModal = function (modal) {
    modal = _(modal);
    if (_('.modal-overlay').length === 0) {
        var overlay = document.createElement('div');
        overlay.className = 'modal-overlay';
        _('body').append(overlay);
    }
    ...
    _('.modal-overlay').addClass('modal-overlay-visible');
    _(modal).addClass('modal-in');
    return true;
};
app.closeModal = function (modal) {
    modal = _(modal || '.modal-in');
    _('.modal-overlay').removeClass('modal-overlay-visible');
    modal.trigger('close');
